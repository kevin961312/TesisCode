...
SimulationT2Chart <- function(observation, numVariables, numSimulation, meanVector, sigmaMatriz, alphaMRCD = 0.75, typeMethod = "MRCD", dfreedom = 1) {
  T2Matrix <- c()
  T2Total <- c()
  T2Max <- c()
  ...
  for (i in 1:numSimulation)
  {
    ...
    if (typeMethod == "MRCD") {
      ...
      T2 <- c()
      CovKMRCD <- CovKmrcd(x = Data, alpha = alphaMRCD)
...
SimulationT2ChartOutliers <- function(observation, numVariables,
                                      ...
                                      sigmaMatriz, shiftMean, percentoutliers = 0, alphaMRCD = 0.75, typeMethod = "MRCD", dfreedom = 1) {
  T2Matrix <- c()
  T2Total <- c()
  T2Max <- c()
