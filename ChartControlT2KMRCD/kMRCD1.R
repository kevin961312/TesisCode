...
source(here::here("Downloads/Tesis/TesisCode/ChartControlT2KMRCD/utils.R"))
...
for (index in 1:n_sol){
  if( length(solution[[index]]$outlyingnessIndices)==0){
    solution[[index]]$outlyingnessIndices = 1:n
  }
...
rhoL <- na.omit(rhoL[rhoL <= max(0.1, median(rhoL), na.rm = TRUE)])
rho <- max(rhoL)
