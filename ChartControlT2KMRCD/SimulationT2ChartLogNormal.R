...
typeMethod = "MRCD", beta = 1, KMRCDModel= "RbfKernel") {
  ...
  {
    Data <- rlnorm.rplus(observation,rep(rho,numVariables),sigma)
    if (typeMethod == "MRCD") {
...
SimulationT2ChartOutliers <- function(observation, numVariables,
  ...
  for (i in 1:numSimulation)
  {
    ...
    if (rhoShift == 0) {
      ...
      Data <- rlnorm.rplus(observation - NumOutliers,rep(rho,numVariables),sigma)
      if (NumOutliers >= 1) {
        DataOutlier <- rlnorm.rplus(NumOutliers,rep(rho + rhoShift,numVariables),sigma)
