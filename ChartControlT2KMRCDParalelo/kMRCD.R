...
source(here::here("Downloads/Tesis/TesisCode/ChartControlT2KMRCDParalelo/utils.R"))
...
CovKmrcd <- function(x, kModel = 'Linear', alpha = 0.75){
  ...
  for (index in 1:n_sol){
    ...
    flagwhile = 1
    while(length(solution[[index]]$outlyingnessIndices)== 0){
      if (length(solution[[flagwhile]]$outlyingnessIndices)== 0){
       flagwhile = flagwhile + 1
      }else {
         solution[[index]]$outlyingnessIndices = solution[[flagwhile]]$outlyingnessIndices
      }
    }
