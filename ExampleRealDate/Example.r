...
library(readr)
gasoline <- read_csv("/Users/kevin.pineda/Downloads/Tesis/TesisCode/ExampleRealDate/gasolineData.csv")
mu <- colMeans(gasoline)
cova <- cov(gasoline)
diago <- diag(cova)
gasoline[c(1,2,3,4,5,6),] <- mvrnorm(6, mu = colMeans(gasoline) + 1*diago, Sigma = cov(gasoline))
...
text(xGasoline,T2TotalG, labels=rownames(gasoline), cex=0.9, font=2)
# dev.off()
